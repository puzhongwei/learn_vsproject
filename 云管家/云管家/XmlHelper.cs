...
namespace 云管家
{
    public class XmlHelper
    {
        ...
        /// <returns></returns>
        public bool DeleteXmlNodeByXPath(string xfilepath,string name,int i,string xpath)
        {
            bool isSuccess = false;
            xmldoc = new XmlDocument();
            try
            {
                xmldoc.Load(xfilepath); //加载XML文档
                 XmlNode pxmlNode = xmldoc.SelectSingleNode(xpath);
                    foreach (XmlNode xn in pxmlNode.ChildNodes)
                    {
                        if (xn.ChildNodes[0].InnerText.Equals(name))
                        {
                            pxmlNode.RemoveChild(xn);
                        }
                    }
              
                xmldoc.Save(xfilepath); //保存到XML文档
                isSuccess = true;
            }
            catch (Exception ex)
            {
        ...
        /// <returns></returns>
        public XmlNodeList GetFolderChild(string foldername)
        {
            string path =@"D:\云管家\config\fileinfo.xml";
            ...
            try
            {
                ...
                XmlNode pxmlNode = xmldoc.SelectSingleNode("fileroot/folders");
                
                foreach (XmlNode xn in pxmlNode.ChildNodes)
                {
                    int i = xn.ChildNodes.Count;
                   
                    if (xn.ChildNodes[0].InnerText.Equals(foldername))
                    {
                        //XmlNodeList xmlNodeList = xn.ChildNodes;
                        foreach (XmlNode xm in xn.ChildNodes)
                        {
                            if (xm.FirstChild.Name.Equals("file"))
                            {
                                return xm.ChildNodes;
                            }
